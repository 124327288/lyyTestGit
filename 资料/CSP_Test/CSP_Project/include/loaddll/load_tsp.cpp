...
//#ifdef WIN32
    ...
    if((void *)0 == m->hModule)
    {
		...
		BYTE OutputDebug[512] = {0};
		//UINT32 Ord = Decode_UINT32(pTransmitBuf+6);
		sprintf((char *)OutputDebug,"YGH加载Jtsp动态库失败:%d",erro);
		MessageBox(NULL, (char *)OutputDebug, "Error", MB_OK);
        return TCSERR(TSM_E_FAIL);
    }
