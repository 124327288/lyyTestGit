		...
		)
{
	...
	DWORD tmpLen, dwHashLen;
	BOOL bRet = pHash->GetValue(NULL,&dwHashLen);
	if (bRet == FALSE)
		return FALSE;
	
	tmpLen = max(dwHashLen, (m_ulKeyLen/8+m_ulSaltLen/8));
	BYTE * tmp = new BYTE[tmpLen];
	bRet = pHash->GetValue(tmp,&dwHashLen);
	if (bRet == FALSE)
	{
		delete tmp;
		return FALSE;
	...
	if (dwHashLen < (m_ulKeyLen/8+m_ulSaltLen/8))
	{
		BYTE pbHashTemp[64] = {0};

		BYTE pbB1[64] = {0};
		memset(pbB1, 0x36, 64);
		xorbuf(pbB1, tmp, dwHashLen);

		BYTE pbB2[64] = {0};
		memset(pbB2, 0x5C, 64);
		xorbuf(pbB2, tmp, dwHashLen);

		CCSPHashObject hashB1(pHash->GetAlgId());
		CCSPHashObject hashB2(pHash->GetAlgId());
		if(!hashB1.HashData(pbB1, 64, 0) || !hashB1.GetValue(pbHashTemp, &dwHashLen))
		{
			SETLASTERROR(NTE_BAD_HASH);
			return FALSE;
		}
		if(!hashB2.HashData(pbB2, 64, 0) || !hashB2.GetValue(pbHashTemp + dwHashLen, &dwHashLen))
		{
			SETLASTERROR(NTE_BAD_HASH);
			return FALSE;
		}

		memcpy(tmp, pbHashTemp, tmpLen);	
	}
