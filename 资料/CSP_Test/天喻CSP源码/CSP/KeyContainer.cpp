	...
	)
{
	...
	//encode the name
	CopyToByteArray((LPBYTE)(LPCTSTR)m_szName, m_szName.GetLength(), encodeArray);
	...
	)
{
	...
	*phKey = NULL;
	...
	if(bIsKeyPair){
		...
		if(dwFlags & CRYPT_USER_PROTECTED){
			...
			if(g_theTYCSPManager.IsSSO())
				Force = !m_pCSPObject->IsCalledLogin();
			if(!m_pCSPObject->Login(Force)){
				SETLASTERROR(NTE_PERM);
				return FALSE;
			}
		}
	...
	//产生密钥对时显示提示等待界面
	CGenKeyPairPromptDlg* genKeyPairPromptDlg = NULL;
	if(bIsKeyPair && !m_pCSPObject->IsSilent()){
		genKeyPairPromptDlg = new CGenKeyPairPromptDlg();
		if(genKeyPairPromptDlg)
			genKeyPairPromptDlg->ShowPrompt();
	}
	...
	//关闭提示等待界面
	if(genKeyPairPromptDlg){
		genKeyPairPromptDlg->HidePrompt();
		delete genKeyPairPromptDlg;
	}
	...
	)
{
	...
	//以下代码专为修正Office XP和Office 2003中的数字签名BUG
	//
	//	Office的数字签名始终查找加密密钥对(哪里在证书里注册的是签名密钥对)
	{
		if(pPrivateKey == NULL){
			char szAppName[MAX_PATH];
			GetModuleFileName(NULL, szAppName, MAX_PATH);
			strupr(szAppName);
			if(strstr(szAppName, "WINWORD") || 
				strstr(szAppName, "EXCEL") ||
				strstr(szAppName, "POWERPNT")){
				if(dwKeySpec == AT_KEYEXCHANGE)
					pPrivateKey = GetKeyPairObjectByAlgId(CALG_RSA_SIGN);
			}
