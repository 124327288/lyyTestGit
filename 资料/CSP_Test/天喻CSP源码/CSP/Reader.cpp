...
void CCSPReader::SetIndex(int nIndex)
{
	...
	if(GetType() == RT_USBPORT)
		m_szName.Format("%s %d", g_szTYKEYNAMEBASE, nIndex);
	else if(GetType() == RT_COMPORT)
		m_szName.Format("%s %d", g_szTYREADERNAMEBASE, nIndex);
}
...
CCSPReader::EstablishContext()
{
	...
	LONG lResult = g_SCardFuncHolder.m_listFunc.pfnSCardEstablishContext(
		SCARD_SCOPE_USER , NULL, NULL, &m_hSC
		);
...
CCSPReader::ConnectCard(BOOL bCheckCardValid)
{
	...
	if(GetType() == RT_PCSC){
		...
		{
			//打开读卡器
			int hReader = g_TYReaderFuncHolder.m_listFunc.pfnTY_Open(57600, m_nIndex);
...
CCSPReader::CheckCardIsExist()
{
	...
	if(GetType() == RT_PCSC){
			...
			g_TYKeyFuncHolder.m_listFunc.pfnTYKey_CardExist)
		{
			//判断卡片是否存在
			TYKEYHANDLE hTYKey;
			TYKEYSTATUS sw = g_TYKeyFuncHolder.m_listFunc.pfnTYKey_OpenTYKey(m_nIndex, &hTYKey);
		...
		{
			int hReader = g_TYReaderFuncHolder.m_listFunc.pfnTY_GetOpenedHandle(m_nIndex);
			if(hReader){
	...
	)
{
	...
	bRetVal = TokenInfoDERDecoding(pbBuffer, dwBufferLen, pTokenInfo);
	///////////////////////////////////////////////////////////////////////
	//读卡片硬件序列号替换
	HWReadCardSN(pTokenInfo->serialNumber, sizeof(pTokenInfo->serialNumber));
	delete pbBuffer;
...
BOOL CCSPReader::HWReadCardSN(char *szSN, int nMaxLen)
{
	...
	memcpy(pbCmd, "\x80\xd8\x00\x00\x10\x01\x02\x03\x04\x05\x06\x07\x08\xec\xd0\x70\xac\xc7\x1a\x8c\x5b", 21);
	if(!SendCommand(pbCmd, 21, pbResp, &dwRespLen))
		return FALSE;
	...
	for(DWORD dwI = 3; dwI < dwRespLen; dwI++)
		sprintf(szSnTemp + dwI * 2, _T("%02x"), pbResp[dwI]);

	memset(szSN, 0, nMaxLen);
	memcpy(szSN, szSnTemp, nMaxLen);
	return TRUE;
}
	...
	)
{
	...
	dwOffset += 2;
	BYTE flag = pfiletable[dwOffset+1] & 0xfe;
	if(bDeleted)
		flag |= FILE_UNUSED;
	else
		flag |= FILE_USED;
	...
	if(!OpenFile(g_cPathTable.fileTablePath, &hFile, NULL))
		return FALSE;
	BOOL bRetVal = WriteFile(hFile, &flag, 1, dwOffset+1);
	CloseFile(hFile);
	if(bRetVal)
		pfiletable[dwOffset+1] = flag;
