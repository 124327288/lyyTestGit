...
#define TYKEY_LED_OFF		0x00	// LED OFF
#define TYKEY_LED_ON		0x01	// LED ON
...
// Cold reset the specified TYKey SIM Slot
TYKEYINT_API TYKEYSTATUS _stdcall TYKey_SIM_ColdReset(
	IN	TYKEYHANDLE		hKey,					// TYkey handle
	OUT	int				*nATRLen,				// ATR length
	OUT unsigned char	*pATRContext			// ATR Context
	);

// Get ATR for the specified TYKey. Before calling this routine, there must be
// a call to TYKey_ColdRest
TYKEYINT_API TYKEYSTATUS _stdcall TYKey_GetATR(
	IN	TYKEYHANDLE		hKey,					// TYkey handle
	OUT	int				*nATRLen,				// ATR length
	OUT unsigned char	*pATRContext			// ATR Context
	);
...
// Get ATR for the specified TYKey. Before calling this routine, there must be
// a call to TYKey_ColdRest
TYKEYINT_API TYKEYSTATUS _stdcall TYKey_SIM_GetATR(
	IN	TYKEYHANDLE		hKey,					// TYkey handle
	OUT	int				*nATRLen,				// ATR length
	OUT unsigned char	*pATRContext			// ATR Context
	);
...
// Send command to the specified TYKey SIM Slot
TYKEYINT_API TYKEYSTATUS _stdcall TYKey_SIM_SendCommand(
	IN	TYKEYHANDLE		hKey,					// TYkey handle
	IN	int				nCommandLen,			// Command length
	IN	unsigned char	*pCommandContext,		// Command context
	OUT	int				*nResponseLen,			// Response length (-1: Operation Failed)
...
TYKEYINT_API void _stdcall TYKey_Status( 
	IN UCHAR*			keyStatus 
	);
...
// Control LED on/off
TYKEYINT_API int _stdcall TYKey_LEDOnOff( 
	IN TYKEYHANDLE		hKey,					// TYkey handle
	IN int				LEDStatus				// LED status
	);

// Control LED flicker
TYKEYINT_API int _stdcall TYKey_LEDFlicker(
	IN TYKEYHANDLE		hKey,					// TYkey handle
	IN int				intervalTime,			// interval time (ms)
	IN int				totleTime				// Totle time	 (ms)
	);
...
#define STATUS_TYKEY_NO_ATR			0x8404		// no ATR
...
#define TY_NO_KEY			0	
#define	TY_SMART_KEY		1
#define TY_SMART_KEY_PRO	2
#define AT_BOOTLOADER		4
