...
CCSPSCB2Key::CCSPSCB2Key():CCSPSCB2Base()
{
}



CCSPSCB2Key::CCSPSCB2Key(
			 CCSPKeyContainer* pKeyContainer,
			 ULONG ulAlgId,
			 BOOL bToken	// = FALSE
			 //BOOL bExtractable = TRUE,
			 //BOOL bPrivate = FALSE
			 ):CCSPSCB2Base(pKeyContainer,ulAlgId,bToken)
{
}
...
CCSPSCB2Key::~CCSPSCB2Key()
{
	CCSPSCB2Base::~CCSPDesTmpl();
}
BOOL CCSPSCB2Key::Encrypt(
	...
	)
{
	...
	//安装SCB2密钥	32字节，分两次装入
	memcpy(cCommand, "\x80\xD4\x01\x00\x18\x01\x01\x03\x0A\x0F\x00\x0F\xFF", 13);
	memcpy(cCommand + 13, m_arbKeyContent.GetData(), 16);
	BOOL bRetVal = pCSPObject->SendCommand(cCommand, 13 + 16, NULL, NULL, &wSW);
	memcpy(cCommand, "\x80\xD4\x01\x00\x18\x02\x01\x04\x0A\x0F\x00\x0F\xFF", 13);
	memcpy(cCommand + 13, m_arbKeyContent.GetData()+16, 16);
	bRetVal = pCSPObject->SendCommand(cCommand, 13 + 16, NULL, NULL, &wSW);
	//老的文件系统中没有预先安装SCB2密钥
	if(bRetVal == FALSE){
		if(wSW != 0x9403) return FALSE;
		memcpy(cCommand, "\x80\xD4\x00\x00\x18\x01\x01\x03\x0A\x0F\x00\x0F\xFF", 13);
		memcpy(cCommand + 13, m_arbKeyContent.GetData(), 16);
		if(!pCSPObject->SendCommand(cCommand, 13 + 16))
			return FALSE;
		memcpy(cCommand, "\x80\xD4\x00\x00\x18\x02\x01\x04\x0A\x0F\x00\x0F\xFF", 13);
		memcpy(cCommand + 13, m_arbKeyContent.GetData()+16, 16);
		if(!pCSPObject->SendCommand(cCommand, 13 + 16))
			return FALSE;
	}
	...
	{
		m_pDESEncryption = new SCB2_Encryption(m_arbKeyContent.GetData(),GetCSPObject());
	}
	...
	)
{
	...
	{
		m_pDESDecryption = new SCB2_Decryption(m_arbKeyContent.GetData(),pCSPObject);
	}
