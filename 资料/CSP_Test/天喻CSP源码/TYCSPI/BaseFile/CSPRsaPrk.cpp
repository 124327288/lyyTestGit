	...
	)
{
	...
	//检测输入数据长度,最大不能超过模长-11
	if(dwDataLen > m_dwBitLen / 8 - 11){
	...
	//填充
	BYTE pbPadedData[256]; //至少2048位
	PKCS_SignaturePaddingScheme pad;
	pad.Pad(g_rng, pbData, dwDataLen, pbPadedData, m_dwBitLen-1);
	...
	if(bRetVal){
		*pdwSigLen = m_dwBitLen / 8;
		memcpy(pbSignature, pbPadedData, *pdwSigLen);
		SwapInt(pbSignature,*pdwSigLen);
	}
		...
		BYTE* pPath)
{
	...
	ulOffset += 2;
	ulOffset += 19;
	bRet = GetCSPObject()->WriteFile(hFile, pPath, 2, ulOffset);
	GetCSPObject()->CloseFile(hFile);
	if (bRet == FALSE)
	{
		GetCSPObject()->EndTransaction();
	...
	m_CertPath[0] = pPath[0];
	m_CertPath[1] = pPath[1];
		...
		)
{
	...
	else if (dwParam == KP_CERTIFICATE)
	{
		...
		if (pbData == NULL)
		{
			...
			{
				GetCSPObject()->DeleteFile(m_CertPath);
				BYTE flag = FILETYPE_CERT|FILE_UNUSED;
...
{
	return CCSPRsaPuk::RSARawEncrypt(pbInData, dwInDataLen, pbOutData, pdwOutDataLen);
}

BOOL CCSPRsaPrk::RSARawDecrypt(
	LPBYTE pbInData,
	DWORD dwInDataLen,
	LPBYTE pbOutData,
	LPDWORD pdwOutDataLen
	)
{
	...
	*pdwOutDataLen = m_dwBitLen / 8;
	...
	BOOL bRetVal = TRUE;
	if (IsNeedHWCalc())
		bRetVal = HWRawRSADecryption(pbOutData);
	else
		bRetVal = SWRawRSADecryption(pbOutData);
