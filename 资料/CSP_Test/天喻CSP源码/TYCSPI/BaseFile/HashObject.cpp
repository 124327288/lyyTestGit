	...
	)
{
	...
	if(dwParam == HP_HASHVAL){
		if(pbData != NULL){
			//判断指向的内存是否合法
			BOOL bIsValid = __IsValidAddress(pbData, m_dwSize, TRUE);
			if(!bIsValid){
...
//	}
	...
	if(m_idAlg == CALG_SSL3_SHAMD5){
		BYTE digest[16 + 20];
		MD5 hashMd5;
		hashMd5.CalculateDigest(digest, pbData, static_cast<unsigned int>(dwDataLen));
		SHA hashSha;
		hashSha.CalculateDigest(digest + 16, pbData,static_cast<unsigned int>(dwDataLen));
		SetValue(digest);
	}
	else{
	...
	)
{
	...
	//Little Endian
	pKey->GetKeyMaterial(pbKeyMaterial, &dwLength);
